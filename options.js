(()=>{"use strict";(()=>{/**
 * Store a key-value pair in Chrome sync storage and mirror or remove a local fallback entry based on sync result.
 * @param {string} key - The storage key to set.
 * @param {*} value - The value to store for the given key.
 * @returns {Promise<void>} Resolves with no value when the storage operations complete.
 */
function e(e,t){return new Promise((n=>{const i={};i[e]=t,chrome.storage.sync.set(i,(()=>{chrome.runtime.lastError?(console.log(chrome.runtime.lastError.message),chrome.storage.local.set(i,(()=>n()))):(chrome.storage.local.remove(e),n())}))}))}/**
 * Load values for the given storage keys and merge sync and local results.
 *
 * Merges values retrieved from chrome.storage.sync with those from chrome.storage.local and resolves with the combined object. If the same key exists in both places, the value from local storage overrides the value from sync storage.
 * @param {string|string[]|Object} keys - A key, array of keys, or a key-to-defaults object accepted by chrome.storage.get.
 * @returns {Object} An object mapping keys to their merged stored values (local overrides sync for duplicate keys).
 */
function t(e){return new Promise((t=>{chrome.storage.sync.get(e,(n=>{chrome.storage.local.get(e,(e=>{const i={...n,...e};t(i)}))}))}))}const n="autopilotActions";let i=[];/**
 * Create a new action object, append it to the in-memory actions list, and return it.
 *
 * The created action includes a sequential `number` (one greater than the current list length)
 * and an `id` — a generated unique hexadecimal identifier.
 *
 * @returns {{id: string, number: number}} The newly created action object with `id` and `number` fields.
 */
function o(){const e={number:i.length+1,id:([1e7]+-1e3+-4e3+-8e3+-1e11).replace(/[018]/g,(e=>(e^crypto.getRandomValues(new Uint8Array(1))[0]&15>>e/4).toString(16)))};return i.push(e),e}/**
 * Load stored actions and return the current in-memory actions array.
 *
 * Loads the value stored under the main actions key and, if it is an array,
 * replaces the in-memory actions list with that value before returning it.
 * @returns {Array<Object>} The current actions array (each action object contains at least `id` and `number`).
 */
async function a(){const e=await async function(){return(await t([n]))[n]}();return Array.isArray(e)&&(i=e),i}/**
 * Save the in-memory actions array to storage under the module's autopilotActions key.
 * @returns {Promise<void>} Resolves when the actions array has been persisted.
 */
